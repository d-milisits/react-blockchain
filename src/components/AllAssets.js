...
const AllAssets = ({ loading }) => {
  ...
  useEffect(() => {
    const fetchCoins = async () => {
      try {
        ...
        // Handle the error state here, e.g., setError(true) or setError(error.message)
      }
    };
  ...
  function sortByCoinCategory(obj) {
    const tempArr = handleSearch().sort((a, b) =>
      obj.order === "asc"
        ? a[obj.apiKey] - b[obj.apiKey]
        : b[obj.apiKey] - a[obj.apiKey]
    );
    ...
    <div className="assets-loaded">
      <div className="all-assets-options-ctr">
        <div className="all-assets-options">
          <div
            onClick={() => {
              changeCoinCategory("All");
            }}
            className={`option ${filterOption === "All" ? "active" : ""}`}
          >
            All Assets
          </div>
          <div
            onClick={() => {
              changeCoinCategory("Gainers");
            }}
            className={`option ${filterOption === "Gainers" ? "active" : ""}`}
          >
            Gainers
          </div>
          <div
            onClick={() => {
              changeCoinCategory("Losers");
            }}
            className={`option ${filterOption === "Losers" ? "active" : ""}`}
          >
            Losers
          </div>
        </div>
      ...
      <div className="all-assets" id="all-assets">
        <div className="all-assets-labels">
          ...
          <div style={{ width: "73.08px", marginLeft: "25px" }}></div>
        </div>
        {handleSearch().length ? (
          handleSearch()
            ...
            .map((coin) => {
              return (
                ...
                >
                  <CoinRow key={coin.name} coin={coin} symbol={symbol} />
                </div>
              );
            })
        ) : (
